...
bool checkDucKIsHit()
{
	...
	if (mouse[0].x != -1 && mouse[0].y != -1)
	{
		...
		{
			score += 10;
			if (score == 100 || score == 1000 || score == 10000) // lam cho co chu cha ai choi toi muc nay
				dis_score += 10;
			return true;
		}
		if (calDis(viTri[1], mouse[0]) > to)
...
int duckFly()
{
	...
	while (bullet > 0)
	{
		...
		{
			if (viTri[i].x >= 1000) //cái dtuong nó chạm x , random theo hướng con vịt theo hướng khác
			{
				stepx[i] = -rand() % 10 + 0;
...
void rules_interface()
{
	...
	char c[100] = "Conversely, if you hit the bomb, you will lose 20 points.";
	outtextxy(x, y + 90, c);

	char d[100] = "When your score is 0, you miss or hit a bomb,";
	outtextxy(x, y + 150, d);
